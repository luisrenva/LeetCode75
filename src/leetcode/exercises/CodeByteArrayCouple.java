...
//		System.out.println(test(arr6)); // Output: "1, 2, 3, 4"
	...
	public static String arrayCouples(int[] arr) {
		...
		for (int i = 0; i < arr.length; i += 2) {
			String pair = arr[i] + COMMA + arr[i + 1];//tomamos los primeros dos y creamos un key y su inverso
			String reversePair = arr[i + 1] + COMMA + arr[i];
		...
		for (Map.Entry<String, Integer> mapValues: pairCounts.entrySet()) {
			if (mapValues.getValue() == 1 ) {
				...
				responseInCaseError += responseInCaseError.length() > 1 ? COMMA : "";
			}
		}

		if (!responseInCaseError.equals("")) {
			if(responseInCaseError.endsWith(COMMA)) {
